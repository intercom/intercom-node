...
export default class Snippet {
    constructor(settings) {
        ...
        if (!settings.app_id) {
            throw new Error(
                'You must provide an app_id in your Intercom settings'
            );
        }
        if (!this.loggedOut && !settings.verificationSecret) {
            throw new Error(
                'You must provide your verification secret in your Intercom settings'
            );
        }
    ...
    settingsToString(settings) {
        ...
        Object.keys(settings).map((key) => {
            if (typeof settings[key] === 'object' && settings[key] !== null) {
                intercomSettings.push(
                    `${key}: { ${this.settingsToString(settings[key])} }`
                );
            } else {
