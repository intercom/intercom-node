...
export class Conversations {
    ...
    ): Promise<Intercom.PaginatedResponse> {
    ...
    public async update(
        ...
        const { display_as: displayAs, ..._body } = request;
        const _queryParams: Record<string, string | string[] | object | object[]> = {};
        if (displayAs != null) {
            _queryParams["display_as"] = displayAs;
        }
    ...
    public async search(
        ...
        const list = async (request: Intercom.SearchRequest): Promise<Intercom.ConversationList> => {
            const _response = await (this._options.fetcher ?? core.fetcher)({
                url: urlJoin(
            ...
            if (_response.ok) {
                return _response.body as Intercom.ConversationList;
            }
            if (_response.error.reason === "status-code") {
                throw new errors.IntercomError({
                    statusCode: _response.error.statusCode,
     ...
     */
    public async assign(id: string, requestOptions?: Conversations.RequestOptions): Promise<Intercom.Conversation> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.IntercomEnvironment.UsProduction,
                `conversations/${encodeURIComponent(id)}/run_assignment_rules`
            ),
            method: "POST",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
