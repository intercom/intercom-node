...
describe('userData', () => {
    ...
    it('should escape bad stuff in values', () => {
        ...
        assert.equal(
            userData.json().email,
            'jess@\\"&lt;script&gt;alert(1)&lt;/script&gt;intercom.io'
        );
    });
    ...
    it('should escape bad stuff in object keys', () => {
        ...
        settings['<script>doSomethingEvil();</script>'] =
            'jess@"<script>alert(1)</script>intercom.io';
        const userData = new UserData(settings);
        const result = userData.json();
        assert.equal(
            result['&lt;script&gt;doSomethingEvil();&lt;/script&gt;'],
            'jess@\\"&lt;script&gt;alert(1)&lt;/script&gt;intercom.io'
        );
    });
    ...
    it('should not include the verification secret in the userData', () => {
        ...
        const result = userData.json();
        assert.equal(
            Object.keys(result).indexOf(settings.verificationSecret),
            -1
        );
    });
    ...
    it('should return the userData', () => {
        ...
        assert.equal(
            ...
            JSON.stringify({
                ...
                user_hash:
                    'f02877f24c9dd37542268a28627ebaf2e07d0d114d9482abcdc20f60874b40b3',
            })
        );
    });
});
