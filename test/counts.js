...
describe('counts', () => {
    ...
    it('conversation admin counts', async () => {
        nock('https://api.intercom.io')
            .get('/counts')
            .query({ type: 'conversation', count: 'admin' })
            .reply(200, {});
        const client = new Client('foo', 'bar');
    ...
    it('company segment counts', async () => {
        nock('https://api.intercom.io')
            .get('/counts')
            .query({ type: 'company', count: 'segment' })
            .reply(200, {});
        const client = new Client('foo', 'bar');
