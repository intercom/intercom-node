...
describe('tags', () => {
  ...
  it('should tag contacts', async () => {
    ...
    nock('https://api.intercom.io').post(`/contacts/${contactId}/tags`, requestBody).reply(200, {});
    const client = new Client('foo', 'bar');
    const response = await client.tags.tagContact({ contactId, tagId: requestBody.id });
  ...
  it('should untag contacts', async () => {
    ...
    nock('https://api.intercom.io').delete(`/contacts/${contactId}/tags/${tagId}`).reply(200, {});
    const client = new Client('foo', 'bar');
