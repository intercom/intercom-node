...
describe('admins', () => {
  ...
  it('should set an admin away', async () => {
    ...
    nock('https://api.intercom.io').put(`/admins/${id}/away`, requestData).reply(200, {});
    const client = new Client('foo', 'bar');
    const response = await client.admins.away({adminId: id, enableAwayMode: true, enableReassignMode: false});
  ...
  it('should list all activites logs', async () => {
    ...
    nock('https://api.intercom.io').get(`/admins/activity_logs?created_at_after=${dateInUnix}&created_at_before=${dateInUnix}`).reply(200, {});
    const client = new Client('foo', 'bar');
    const response = await client.admins.listAllActivityLogs({before: date, after: date});
