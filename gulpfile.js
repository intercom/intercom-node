...
gulp.task('compile_ts', () => {
    ...
    return tsResult.js.pipe(sourcemaps.write('.')).pipe(gulp.dest('./lib'));
});
...
gulp.task('babel', function () {
    return gulp.src('lib/**/*.js').pipe(babel()).pipe(gulp.dest('dist'));
});
...
gulp.task(
    ...
    gulp.series(
        'static',
        'compile_ts',
        'generate_type_declarations_ts',
        'babel'
    )
);
gulp.task(
    'default',
    gulp.series('static', 'compile_ts', 'compile_tests_ts', 'test')
);
